...
import (
	...
	"go-galaxy/cmd/galaxy/launcher/metrics"
	"go-galaxy/cmd/galaxy/launcher/tracing"
	"go-galaxy/debug"
	"go-galaxy/evmcore"
	"go-galaxy/flags"
	"go-galaxy/gossip"
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	svc, err := gossip.NewService(stack, cfg.Galaxy, gdb, blockProc, engine, dagIndex, newTxPool)
	if err != nil {
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call deam_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "deam_syncing")
				if err != nil {
					continue
				}
				if !syncing {
