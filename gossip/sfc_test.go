...
import (
	...
	"go-galaxy/gossip/contract/driver100"
	"go-galaxy/gossip/contract/driverauth100"
	"go-galaxy/gossip/contract/netinit100"
	"go-galaxy/gossip/contract/sfc100"
	"go-galaxy/logger"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr, err := env.ApplyTxs(nextEpoch,
				env.Contract(admin, utils.ToUnit(0), sfc100.ContractBin),
			)
			require.NoError(err)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr, err := env.ApplyTxs(nextEpoch,
			env.Contract(admin, utils.ToUnit(0), anyContractBin),
		)
		require.NoError(err)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
...
func circleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := idx.Validator(0); i < validatorsNum; i++ {
			...
			txs[i] = env.Transfer(idx.ValidatorID(from+1), idx.ValidatorID(to+1), utils.ToUnit(100))
		}
