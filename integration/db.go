...
import (
	...
	"github.com/deamchain/deam-v2-base/kvdb/leveldb"
...
func (db *DBProducerWithMetrics) OpenDB(name string) (kvdb.DropableStore, error) {
	...
	dm.log = logger
	dm.diskReadMeter = metrics.GetOrRegisterMeter("galaxy/chaindata/"+name+"/disk/read", nil)
	dm.diskWriteMeter = metrics.GetOrRegisterMeter("galaxy/chaindata/"+name+"/disk/write", nil)
...
func DBProducer(chaindataDir string, scale cachescale.Func) kvdb.IterableDBProducer {
	if chaindataDir == "inmemory" || chaindataDir == "" {
		chaindataDir, _ = ioutil.TempDir("", "galaxy-integration")
	}
